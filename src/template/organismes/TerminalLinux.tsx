...
// Typage des fichiers/dossiers
type FileNode = {
  type: 'dir' | 'file';
  content?: { [key: string]: FileNode }; // index signature pour accès dynamique
  data?: string; // contenu pour les fichiers
};
...
const TerminalLinux = () => {
      ...
      content: {
        home: {
          ...
          content: {
            'readme.txt': {
              type: 'file',
              data: 'Bienvenue dans mon terminal personnalisé !',
            },
          },
        },
        var: {
  ...
  const commands = {
    ...
    touch: (args: string[]) => {
      ...
      if (!dirNode || dirNode.type !== 'dir') return `touch: impossible de créer '${args[0]}'`;
      if (dirNode.content?.[fileName]) return '';
      // création fichier vide
      const newFs = { ...fs };
      let node = newFs['/'];
      for (const key of dirPath.split('/').filter(Boolean)) {
        node = node.content![key];
      }
      node.content![fileName] = { type: 'file', data: '' };
      setFs(newFs);
      return '';
    },
    ...
    mkdir: (args: string[]) => {
      ...
      if (!parentNode || parentNode.type !== 'dir') return `mkdir: impossible de créer '${args[0]}'`;
      if (parentNode.content?.[dirName]) return `mkdir: existe déjà`;
      const newFs = { ...fs };
      let node = newFs['/'];
      for (const key of parentPath.split('/').filter(Boolean)) {
        node = node.content![key];
      }
    ...
    rmdir: (args: string[]) => {
      ...
      const parentPath = parts.length === 0 ? '/' : '/' + parts.join('/');
      const parentNode = getNode(parentPath);
      if (!parentNode || parentNode.type !== 'dir') return `rmdir: impossible de supprimer '${args[0]}'`;
      const target = parentNode.content![dirName];
      if (!target || target.type !== 'dir') return `rmdir: ce n'est pas un dossier`;
      if (Object.keys(target.content || {}).length > 0) return `rmdir: dossier non vide`;
      const newFs = { ...fs };
      let node = newFs['/'];
      for (const key of parentPath.split('/').filter(Boolean)) {
        node = node.content![key];
    ...
    rm: (args: string[]) => {
      ...
      const parentPath = parts.length === 0 ? '/' : '/' + parts.join('/');
      const parentNode = getNode(parentPath);
      if (!parentNode || parentNode.type !== 'dir') return `rm: impossible de supprimer '${args[0]}'`;
      const target = parentNode.content![fileName];
      if (!target || target.type !== 'file') return `rm: ce n'est pas un fichier`;
      const newFs = { ...fs };
      let node = newFs['/'];
      for (const key of parentPath.split('/').filter(Boolean)) {
        node = node.content![key];
